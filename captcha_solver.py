...
and would we recognise it if it appeared?", not "did we meet one"):
  ...
  * No reCAPTCHA, hCaptcha, DataDome or PerimeterX markup appears on any
    ...
    the same code is what the rest of the family runs.
...
Two rules follow from that, and both are load-bearing:
  ...
  * **Extension-injected markup is not the site's.** The 2Captcha Scraping
    ...
    2026-08-27 capture contains exactly that, on a page with no challenge on
    it. Every detector here strips `chrome-extension://` / `moz-extension://`
    script tags first (product_parser.strip_extension_scripts).
...
def detect_recaptcha_v3(html: str, page_url: str = "") -> Optional[CaptchaChallenge]:
    ...
    Neither appears on any flippa.com capture; kept because detection stays
